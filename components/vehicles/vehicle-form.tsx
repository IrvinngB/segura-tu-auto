...
export function VehicleForm({ customerId, onSuccess, onCancel }: VehicleFormProps) {
  ...
  const handleFeatureToggle = (feature: string, type: "safety" | "antitheft") => {
    if (type === "safety") {
      setSafetyFeatures((prev) => (prev.includes(feature) ? prev.filter((f) => f !== feature) : [...prev, feature]))
    } else {
      setAntiTheftDevices((prev) => (prev.includes(feature) ? prev.filter((f) => f !== feature) : [...prev, feature]))
    }
  }
    ...
    try {
      const { data, error } = await supabase
        ...
        .insert({
          ...
          estimated_value: vehicleData.estimatedValue ? Number.parseFloat(vehicleData.estimatedValue) : null,
          mileage: vehicleData.mileage ? Number.parseInt(vehicleData.mileage) : null,
          garage_type: vehicleData.garageType,
          annual_mileage: vehicleData.annualMileage ? Number.parseInt(vehicleData.annualMileage) : null,
  ...
  return (
    <Card className="w-full max-w-4xl mx-auto">
      <CardHeader>
        ...
        <CardDescription>Complete la información del vehículo para el seguro</CardDescription>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="space-y-2">
              <Label htmlFor="engineSize">Tamaño del Motor</Label>
              <Input
                id="engineSize"
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="space-y-2">
              ...
              >
                ...
                <SelectContent>
                  ...
                  <SelectItem value="Automático">Automática</SelectItem>
            ...
            <div className="space-y-2">
              ...
              <Select value={vehicleData.usageType} onValueChange={(value) => handleInputChange("usageType", value)}>
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
          ...
          {/* Garage Type */}
          <div className="space-y-2">
            <Label htmlFor="garageType">Tipo de Estacionamiento</Label>
            <Select value={vehicleData.garageType} onValueChange={(value) => handleInputChange("garageType", value)}>
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="enclosed">Garage cerrado</SelectItem>
                <SelectItem value="covered">Cochera techada</SelectItem>
              ...
              {safetyFeatureOptions.map((feature) => (
                <label key={feature} className="flex items-center space-x-2 cursor-pointer">
                  <input
                    ...
                    onChange={() => handleFeatureToggle(feature, "safety")}
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
              {antiTheftOptions.map((device) => (
                <label key={device} className="flex items-center space-x-2 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={antiTheftDevices.includes(device)}
                    onChange={() => handleFeatureToggle(device, "antitheft")}
                    className="rounded border-gray-300"
                  />
          ...
          <div className="flex gap-4 pt-6">
            <Button type="submit" disabled={loading || !vehicleData.make || !vehicleData.model} className="flex-1">
