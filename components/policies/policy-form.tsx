...
export function PolicyForm({ customerId, onSuccess, onCancel }: PolicyFormProps) {
  ...
  const fetchInitialData = async () => {
    try {
      ...
      if (coverageData) {
        ...
        const mandatoryCoverages = coverageData.filter((c) => c.is_mandatory).map((c) => c.id)
  ...
  const calculatePremium = async () => {
    if (!selectedCustomer || !selectedVehicle || selectedCoverages.length === 0) {
    ...
    try {
      ...
      // Get vehicle data
      const { data: vehicle } = await supabase.from("vehicles").select("*").eq("id", selectedVehicle).single()

      // Get selected coverage types
      const { data: coverages } = await supabase.from("coverage_types").select("*").in("id", selectedCoverages)
      ...
      if (customer && vehicle && coverages) {
        ...
        coverages.forEach((coverage) => {
          ...
          if (vehicle.estimated_value) {
            const valueMultiplier = Math.min(vehicle.estimated_value / 200000, 2) // Cap at 2x
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (policyError) {
        ...
        throw new Error(`Error al crear la póliza: ${policyError.message}`)
      }
      ...
      if (coverageInserts.length > 0) {
        ...
        if (coverageError) {
          ...
          throw new Error(`Error al crear las coberturas: ${coverageError.message}`)
        }

        console.log("Coberturas creadas exitosamente:", coverageData)
      }

      setSuccess(`Póliza ${policyNumber} creada exitosamente con ${coverageInserts.length} coberturas`)
      ...
      setError(error instanceof Error ? error.message : "Error inesperado al crear la póliza")
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <Card className="w-full max-w-4xl mx-auto">
      <CardHeader>
        ...
        <CardDescription>Complete la información para crear una nueva póliza de seguro</CardDescription>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <Select value={selectedCustomer} onValueChange={setSelectedCustomer} disabled={!!customerId}>
                <SelectTrigger>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value) => setPolicyData((prev) => ({ ...prev, policyType: value }))}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
                  ...
                  <SelectItem value="basic">Cobertura Básica</SelectItem>
                </SelectContent>
              </Select>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value) => setPolicyData((prev) => ({ ...prev, paymentFrequency: value }))}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <Input
                  ...
                  onChange={(e) => setPolicyData((prev) => ({ ...prev, startDate: e.target.value }))}
                  className="pl-10"
                  required
                />
              </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <Input
                  ...
                  onChange={(e) => setPolicyData((prev) => ({ ...prev, endDate: e.target.value }))}
                  className="pl-10"
                  required
                />
              </div>
                ...
                <div key={coverage.id} className="flex items-start space-x-3 p-4 border rounded-lg">
                  ...
                  <div className="flex-1 space-y-1">
                    ...
                    <div className="text-xs text-muted-foreground">
                      ...
                      {coverage.deductible && <span> | Deducible: ${coverage.deductible.toLocaleString()}</span>}
                    </div>
                  </div>
                </div>
              ))}
              ...
              <CardContent className="pt-6">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-medium">Prima Calculada</span>
                  </div>
                  <div className="text-right">
                    <div className="text-2xl font-bold text-primary">${calculatedPremium.toLocaleString()}</div>
                    <div className="text-sm text-muted-foreground">
                      {policyData.paymentFrequency === "monthly" && `$${(calculatedPremium / 12).toFixed(2)} mensual`}
                      {policyData.paymentFrequency === "quarterly" &&
                        `$${(calculatedPremium / 4).toFixed(2)} trimestral`}
                      {policyData.paymentFrequency === "biannual" && `$${(calculatedPremium / 2).toFixed(2)} semestral`}
                      {policyData.paymentFrequency === "annual" && "anual"}
                    </div>
                  </div>
                </div>
              </CardContent>
          ...
          <div className="flex items-center space-x-2">
            <Checkbox
              ...
              onCheckedChange={(checked) => setPolicyData((prev) => ({ ...prev, autoRenewal: checked as boolean }))}
            />
          ...
          {process.env.NODE_ENV === 'development' && (
            <div className="p-4 bg-muted/50 rounded-lg text-sm">
              ...
              <p>Cliente seleccionado: {selectedCustomer ? 'Sí' : 'No'}</p>
              <p>Vehículo seleccionado: {selectedVehicle ? 'Sí' : 'No'}</p>
              <p>Coberturas seleccionadas: {selectedCoverages.length}</p>
              <p>Loading: {loading ? 'Sí' : 'No'}</p>
              <p>Prima calculada: ${calculatedPremium}</p>
            </div>
          )}
          ...
          <div className="flex gap-4 pt-6">
            <Button
              type="submit"
              disabled={loading || !selectedCustomer || !selectedVehicle || selectedCoverages.length === 0}
              className="flex-1"
            >
