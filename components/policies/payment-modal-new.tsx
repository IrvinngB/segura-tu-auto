...
export function PaymentModal({
    ...
    const fetchPaymentMethods = async () => {
        ...
        try {
            ...
            // Seleccionar el método primario por defecto
            const primaryMethod = simulatedPaymentMethods.find(m => m.is_primary);
            if (primaryMethod) {
        ...
        try {
            ...
            if (result.success && result.paymentId) {
                ...
                const { error: paymentError } = await supabase
                    .from("payments")
                    .insert({
                        policy_id: null, // Se llenará cuando se cree la póliza
        ...
        <>
            <DialogHeader>
                <DialogTitle className="flex items-center gap-2">
                    ...
                    Procesar Pago - ${amount.toLocaleString()}
                </DialogTitle>
                <DialogDescription>
                    Póliza: {policyNumber} • Pago seguro y encriptado
                </DialogDescription>
            </DialogHeader>
