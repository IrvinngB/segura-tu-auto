...
const DEFAULT_CATEGORIES: DocumentCategory[] = [
  {
    id: 'policy',
    name: 'Póliza',
    description: 'Documentos de pólizas de seguro',
  ...
  },
  {
    ...
    description: 'Formularios de reclamaciones',
  ...
  },
  {
    id: 'photo',
    name: 'Fotografías',
    description: 'Fotos de evidencia',
    max_file_size: 20 * 1024 * 1024, // 20MB
    allowed_mime_types: ['image/jpeg', 'image/png', 'image/webp'],
  },
  {
    id: 'repair_estimate',
    name: 'Estimación de Reparación',
    description: 'Cotizaciones y estimaciones',
    max_file_size: 10 * 1024 * 1024,
    allowed_mime_types: ['application/pdf', 'image/jpeg', 'image/png'],
  },
  {
    id: 'police_report',
    name: 'Reporte Policial',
    description: 'Reportes policiales',
    max_file_size: 10 * 1024 * 1024,
    allowed_mime_types: ['application/pdf', 'image/jpeg', 'image/png'],
  },
  {
...
export default function DocumentManagementPage() {
  ...
  const [categories] = useState<DocumentCategory[]>(DEFAULT_CATEGORIES);
  ...
  const fetchDocumentsAndStats = async () => {
    ...
    try {
      ...
      if (categoryFilter !== 'all') {
        query = query.eq('document_type', categoryFilter);
      }
      ...
      if (documentsError) {
        ...
        // Si no hay documentos, no es un error crítico
      ...
      const byType = enrichedDocuments.reduce((acc: Record<string, number>, doc) => {
      ...
      setStats({
        ...
        pending_review: byType['claim_form'] || 0,
        approved: byType['policy'] || 0,
        expired: byType['photo'] || 0,
  ...
  const handleUpload = async () => {
    if (!selectedFile || !uploadForm.category) {
      setMessage({ type: 'error', text: 'Por favor seleccione un archivo y una categoría' });
      return;
    }
  ...
  return (
    <ProtectedRoute allowedRoles={['admin', 'agent', 'adjuster']}>
      <div className="container mx-auto py-8 px-4">
        ...
        {!loading && documents.length === 0 && !searchTerm && statusFilter === 'all' && categoryFilter === 'all' && (
          <Alert className="mb-6 bg-blue-50 border-blue-200">
            <AlertDescription className="text-blue-900">
              <strong>Sistema de Gestión Documental Activo</strong>
              <p className="mt-2 text-sm">
                No hay documentos cargados aún. Los documentos subidos aparecerán aquí automáticamente.
                Puedes subir licencias, cédulas, registros de vehículos, pólizas y evidencias de reclamaciones.
              </p>
            </AlertDescription>
          </Alert>
        )}
              ...
              <CardContent className="text-center py-12">
                ...
                <h3 className="text-lg font-semibold mb-2">
                  {searchTerm || statusFilter !== 'all' || categoryFilter !== 'all'
                    ...
                    : 'Comienza a gestionar documentos'}
                </h3>
                <p className="text-muted-foreground mb-4">
                  {searchTerm || statusFilter !== 'all' || categoryFilter !== 'all'
                    ? 'Intenta ajustar los filtros de búsqueda.'
                    : 'Sube documentos importantes como licencias, cédulas y evidencias de reclamaciones.'}
                </p>
                {!searchTerm && statusFilter === 'all' && categoryFilter === 'all' && (
