...
export default function HomePage() {
    ...
    useEffect(() => {
        const fetchDashboardData = async () => {
            // Si no hay usuario y no estamos cargando, no hacer nada (mostrar landing pública)
            if (!user || !userProfile) {
                if (!authLoading) {
                    // No redirigir, simplemente terminar - la landing pública se mostrará
                    setLoading(false);
                }
                return;
            }
            ...
            try {
                ...
                if (userProfile.role === "customer") {
                    ...
                    if (customerData) {
                        const [policiesResult, claimsResult] =
                            await Promise.all([
                                supabase
                                    .from("policies")
                                    .select("id")
