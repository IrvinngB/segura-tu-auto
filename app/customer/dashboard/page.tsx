...
export default function CustomerDashboard() {
  ...
  const getStatusBadge = (status: string, type: "policy" | "claim" | "payment") => {
    if (type === "policy") {
      ...
      const statusConfig = config[status as keyof typeof config] || { label: status, variant: "outline" as const }
                    ...
                    <div key={policy.id} className="flex items-center justify-between p-4 border rounded-lg">
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-4 mt-2 text-xs text-muted-foreground">
                          ...
                          <span className="flex items-center gap-1">
                            <DollarSign className="h-3 w-3" />${policy.premium_amount.toLocaleString()}
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-4 mt-2 text-xs text-muted-foreground">
                          ...
                          {claim.estimated_damage_cost && (
                            <span className="flex items-center gap-1">
                              <DollarSign className="h-3 w-3" />${claim.estimated_damage_cost.toLocaleString()}
                            </span>
                          )}
                        </div>
                      </div>
                ...
                {payments.map((payment) => (
                  <div key={payment.id} className="flex items-center justify-between p-4 border rounded-lg">
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-muted-foreground">
                        {payment.payment_type === "premium" ? "Prima de seguro" : "Pago de reclamación"}
                      </p>
