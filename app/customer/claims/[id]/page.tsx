              ...
              {claim.approved_amount && (
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <p className="text-lg font-semibold text-green-600">${claim.approved_amount.toLocaleString()}</p>
                  </div>
                </div>
              )}
            </CardContent>
          ...
          <Card className="lg:col-span-2">
            ...
            <CardContent>
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <div className="text-sm text-muted-foreground">
                    ...
                    {claim.status === 'under_review' && (
                      <p>Su reclamación está siendo revisada por nuestro equipo de documentación.</p>
                    )}
                    {claim.status === 'pending_documentation' && (
                      <p>Necesitamos documentación adicional para continuar con su reclamación. Por favor, contacte a su agente.</p>
                    )}
                    {claim.status === 'investigating' && (
                      <p>Su reclamación está siendo evaluada técnicamente por un ajustador especializado.</p>
                    )}
                    {claim.status === 'waiting_approval' && (
                      <p>La evaluación técnica ha sido completada y está pendiente de aprobación final.</p>
                    )}
                    {claim.status === 'approved' && (
                      <p>Su reclamación ha sido aprobada. El proceso de pago iniciará pronto.</p>
                    )}
                    {claim.status === 'processing_payment' && (
                      <p>Su pago está siendo procesado. Recibirá la compensación en los próximos días hábiles.</p>
                    )}
                    {claim.status === 'paid' && (
                      <p>Su reclamación ha sido pagada exitosamente.</p>
                    )}
                    {claim.status === 'denied' && (
                      <p>Su reclamación ha sido denegada. Para más información, contacte a su agente.</p>
                    )}
                    {claim.status === 'closed' && (
                      <p>Su reclamación ha sido cerrada.</p>
                    )}
                  </div>
                </div>
                ...
                {claim.adjuster && (
                  <div>
                    <label className="text-sm font-medium text-muted-foreground">Ajustador Asignado</label>
                    <div className="flex items-center gap-2 mt-1">
                      <User className="h-4 w-4 text-muted-foreground" />
                      <p>{claim.adjuster.first_name} {claim.adjuster.last_name}</p>
                    </div>
                  </div>
                )}
              </div>
