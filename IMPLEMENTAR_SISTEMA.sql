-- SCRIPT COMPLETO PARA IMPLEMENTAR EL SISTEMA DE RECLAMACIONES
-- Ejecutar en Supabase SQL Editor

-- 1. Actualizar restricción de estados en la tabla claims
...
-- 2. Crear tabla para historial de estados
...
-- 4. Actualizar tabla communications para incluir 'internal'
...
-- 6. Crear políticas de seguridad para claim_status_history
...
-- 7. Crear función trigger para registrar cambios automáticamente
...
-- 9. Verificar que todo esté funcionando correctamente
SELECT 'Sistema de reclamaciones implementado correctamente. Todas las tablas y restricciones están listas.' as status;

-- 10. Mostrar las restricciones activas
SELECT 
    tc.constraint_name, 
    tc.table_name, 
    cc.check_clause 
...
WHERE tc.table_name IN ('claims', 'communications') 
    AND tc.constraint_type = 'CHECK';

-- 11. Verificar que las tablas existen
SELECT table_name, table_type 
FROM information_schema.tables 
WHERE table_name IN ('claims', 'claim_status_history', 'communications') 
    AND table_schema = 'public';

-- Mensaje final
SELECT 'IMPLEMENTACIÓN COMPLETA - El sistema está listo para usar con todos los nuevos estados y funcionalidades.' as message;