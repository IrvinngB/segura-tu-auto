...
export class DocumentProcessor {
    ...
    async processDocument(
        ...
        try {
            ...
            const verification = await this.verifyDocument(
                extractedData,
                category
            );

            // Validate extracted data
            const dataValidation = this.validateExtractedData(
                extractedData,
                category
            );
    ...
    private validateExtractedData(
        ...
        switch (category) {
            case "license":
                if (
                    data.licenseNumber &&
                    !/^[A-Z0-9]{8,12}$/.test(data.licenseNumber)
                ) {
            ...
            case "registration":
                if (
                    data.plateNumber &&
                    !/^[A-Z]{3}-[0-9]{3}$/.test(data.plateNumber)
                ) {
