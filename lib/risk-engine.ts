...
export class RiskAssessmentEngine {
  ...
  assessRisk(factors: RiskFactors): RiskAssessmentResult {
    ...
    const experienceScore = this.assessExperience(factors.driver.experience)
    riskFactors.push({
      category: "Conductor",
      factor: "Experiencia",
      ...
      description: this.getExperienceDescription(factors.driver.experience),
    })
    totalScore += experienceScore * this.weights.experience

    // Traffic violations assessment
    const violationsScore = this.assessViolations(factors.driver.violations)
    riskFactors.push({
      category: "Historial",
      factor: "Infracciones",
    ...
    const kmScore = this.assessAnnualKilometers(factors.usage.annualKilometers)
    ...
    riskFactors.push(
      {
        ...
        description: this.getVehicleAgeDescription(factors.vehicle.year),
      },
      {
        category: "Vehículo",
        factor: "Tipo",
    ...
    // Calculate premium
    const premiumMultiplier = this.calculatePremiumMultiplier(normalizedScore)
  ...
  private generateRecommendations(factors: RiskFactors, riskFactors: RiskFactor[]): string[] {
    ...
    if (factors.driver.violations > 0) {
      recommendations.push("Mantener un historial de conducción limpio puede reducir significativamente la prima")
    }

    if (factors.driver.accidents > 0) {
      recommendations.push("Considerar un curso de manejo defensivo para mejorar el perfil de riesgo")
    }

    if (factors.vehicle.year < new Date().getFullYear() - 10) {
      recommendations.push("Actualizar a un vehículo más nuevo con mejores características de seguridad")
    }

    if (factors.usage.annualKilometers > 30000) {
      recommendations.push("Reducir el kilometraje anual puede disminuir el riesgo y la prima")
    }

    if (factors.location.zone.includes("high")) {
      recommendations.push("Considerar estacionamiento seguro o sistema de alarma antirrobo")
    }

    if (recommendations.length === 0) {
      recommendations.push("Su perfil de riesgo es favorable. Mantenga sus buenos hábitos de conducción.")
    }
